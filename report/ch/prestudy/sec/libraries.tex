...
interface to communicate with the server through basic HTTP commandos like GET, POST, PUT and UPDATE. For the app, the communication was done by using the JSON format 
...
\subsubsection{Jetty}
Jetty is an integrated HTTP server that allows the server app to run by itself without the need for any external web server like Apache or Tomcat~\cite{tomcat}. 
This makes setting up and running the server app more user friendly and faster.
...
%The Facebook SDK is used by numerous apps on the Android platform and is very well maintained and documented. 
