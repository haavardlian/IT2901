...
As explained in the previous section, the team analyzed the possible risks that might occur during the time scope of the project. The risk analysis table gives a certain idea about how big of an impact the team suspected a risk to have, but says less about which risks that actually occurred and which of them that had the biggest impact on the project. The following sections gives a short but explanatory description of the most influential risks we experienced throughout the project.

\subsubsection{Underestimation of workload}
A prevalent risk to any project is failure to acknowledge the cost of tasks in the project and the improper allocation of resources that follows such an error. This can also lead to a false sense of what a team can achieve in a given time frame. \todo[inline]{trekk frem sprint 1 som eksempel.}
...
\todo[inline]{pek på seksjonen hvor vi beskriver problemene med Yodiz}
...
\todo[inline]{trekk frem omstrukturering (if applied) i sprint 5}
...
\todo[inline]{trekk frem dette hvis det forårsaker et problem (sprint 5), ny strukturering av hovedmeny++}
